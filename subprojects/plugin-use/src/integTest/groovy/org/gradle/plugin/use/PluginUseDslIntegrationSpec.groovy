...
class PluginUseDslIntegrationSpec extends AbstractIntegrationSpec {
    ...
    private static final String WRONG_SIGNATURE = "No signature of method"
    ...
    def "illegal syntax in plugins block - #code"() {
        ...
        2          | "id(1)"                                | WRONG_SIGNATURE
        2          | "version('foo')"                       | WRONG_SIGNATURE
        2          | "id('foo').version(1)"                 | WRONG_SIGNATURE
        2          | "id 'foo' version 1"                   | WRONG_SIGNATURE
        2          | "id 'foo' bah '1'"                     | WRONG_SIGNATURE
        2          | "foo 'foo' version '1'"                | WRONG_SIGNATURE
        3          | "id('foo')\nfoo 'bar'"                 | WRONG_SIGNATURE
        2          | "id 'foo';version 'bar'"               | WRONG_SIGNATURE
        2          | "file('foo')" /* script api */         | WRONG_SIGNATURE
        2          | "getVersion()" /* script target api */ | WRONG_SIGNATURE
    }
